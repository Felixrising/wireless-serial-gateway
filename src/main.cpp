/*===================================================================================
          ...
          MrDIY Wireless Serial NG - Universal Edition (ESP8266 & ESP32 S3)

  =================================================================================== */

// Platform detection and version
#ifdef ESP32_S3
    #define FRMW_VERSION "1.2236-ESP32S3"
    #define BOARD_TYPE "ESP32-S3"
#elif defined(ESP8266_D1MINI)
    #define FRMW_VERSION "1.2236-ESP8266"
    #define BOARD_TYPE "ESP8266"
#else
    #define FRMW_VERSION "1.2236-UNKNOWN"
...
#ifdef ESP32_S3
    // ESP32 S3 Xiao specific pin definitions
    // LED_BUILTIN is already defined in the board variant
#elif defined(ESP8266_D1MINI)
    // ESP8266 D1 Mini specific pin definitions
    // LED_BUILTIN is already defined for ESP8266
#endif
...
#ifdef ESP32_S3
    void setupHardwareSerial(int rxPin, int txPin, long baudRate);
#elif defined(ESP8266_D1MINI)
...
// Platform-specific server and client definitions
#ifdef ESP32_S3
    WebServer server(80);
    WiFiClient wifiClient;
    WebSocketsServer webSocket = WebSocketsServer(8000);
    
    // Hardware Serial variables for ESP32 (using UART1 and UART2)
    HardwareSerial *hwSerial = nullptr;
    bool useHwSerial = false;
#elif defined(ESP8266_D1MINI)
    ESP8266WebServer server(80);
    WiFiClient wifiClient;
    WebSocketsServer webSocket = WebSocketsServer(8000);
    
    // SoftwareSerial variables for ESP8266
...
struct settings
{
    ...
    char password[64];
    unsigned long baud;
    int rxPin; // For Hardware Serial RX
    int txPin; // For Hardware Serial TX
} gateway_settings = {};
...
#ifdef ESP32_S3
// Function to initialize Hardware Serial with selected pins and baud rate (ESP32)
void setupHardwareSerial(int rxPin, int txPin, long baudRate)
{
    if (hwSerial)
    {
    ...
    // Use UART1 for custom serial communication
    hwSerial = &Serial1;
    hwSerial->begin(baudRate, SERIAL_8N1, rxPin, txPin);
    useHwSerial = true;
}
#elif defined(ESP8266_D1MINI)
// Function to initialize SoftwareSerial with selected pins and baud rate (ESP8266)
void setupSoftwareSerial(int rxPin, int txPin, long baudRate)
{
    if (softSerial)
    {
...
// Modified sendDeviceMessage to support both platforms
void sendDeviceMessage(String msg)
{
#ifdef ESP32_S3
    if (useHwSerial && hwSerial)
    {
        hwSerial->print(msg + "\r\n");
    }
    else
...
void getDeviceMessages()
{
    ...
    int i;
#ifdef ESP32_S3
    while ((useHwSerial ? hwSerial->available() : Serial.available()) > 0)
    {
        if (useHwSerial && hwSerial)
        {
            i = hwSerial->readBytesUntil('\n', buff, sizeof(buff) - 1);
        }
        else
        {
...
void startServer()
{
    ...
    server.on("/boardinfo", handleBoardInfo);
...
void handleMain()
{
    ...
    server.sendHeader("Cache-Control", String("public, max-age=") + cacheDuration);
    server.send_P(200, "text/html", html_template);
}

// Modified handleUpdate to accept RX/TX pins and initialize serial interface
void handleUpdate()
{
    ...
    if (interfaceType == "hw")
    {
        setupHardwareSerial(rxPin, txPin, baud);
    }
    else if (interfaceType == "usb")
    {
        // For USB Serial, clean up any existing hardware serial
        if (hwSerial)
        {
            hwSerial->end();
            hwSerial = nullptr;
        ...
        useHwSerial = false;
        // Don't reinitialize USB Serial - it's already initialized in setup()
    }
#elif defined(ESP8266_D1MINI)
    if (interfaceType == "sw")
    {
    ...
    else if (interfaceType == "hw")
    {
        // For ESP8266 Hardware Serial, clean up any existing software serial
        if (softSerial)
        {
            delete softSerial;
            softSerial = nullptr;
...
void handleBoardInfo()
{
    JsonDocument boardInfo;
    boardInfo["board_type"] = BOARD_TYPE;
    boardInfo["firmware_version"] = FRMW_VERSION;
    
    char response[200];
    serializeJson(boardInfo, response);
...
void handleConfig()
{
    ...
    {
        // Platform-specific flash size detection
#ifdef ESP32_S3
        uint32_t realSize = ESP.getFlashChipSize();
        uint32_t ideSize = ESP.getFlashChipSize(); // ESP32 doesn't have separate IDE size
#elif defined(ESP8266_D1MINI)
        ...
        s += "<body><main>";
        s += "<div class='row justify-content-between'>";
#ifdef ESP8266_D1MINI
        if (ideSize != realSize)
            s += "<div class='alert-danger'>Your flash size (" + String(ideSize) + ") is configured incorrectly. It should be " + String(realSize) + ".</div>";
#endif
...
void setup()
{
...
#ifdef ESP32_S3
    // ESP32 S3 needs a small delay for USB-CDC to initialize
    delay(1000);
#endif
    ...
    if (!(serial_baud > 0 && serial_baud < 115200))
        serial_baud = 115200;
    
#ifdef ESP32_S3
    // For ESP32 S3, use a higher baud rate for USB-CDC if not specified
    ...
    // WiFi.persistent(true); // Not needed for ESP32
    byte tries = 1;
    ...
    // Initialize serial interface based on stored settings
    // Default to USB Serial if no specific settings are stored
#ifdef ESP32_S3
    if (gateway_settings.rxPin > 0 && gateway_settings.txPin > 0) {
        setupHardwareSerial(gateway_settings.rxPin, gateway_settings.txPin, serial_baud);
    } else {
        useHwSerial = false; // Use USB Serial by default
    }
#elif defined(ESP8266_D1MINI)
    if (gateway_settings.rxPin > 0 && gateway_settings.txPin > 0) {
        setupSoftwareSerial(gateway_settings.rxPin, gateway_settings.txPin, serial_baud);
    } else {
        useSoftSerial = false; // Use Hardware Serial by default
    }
#endif
...
void loop()
{
    ...
    ArduinoOTA.handle();
#ifdef ESP8266_D1MINI
    MDNS.update(); // ESP8266 needs explicit MDNS update
#endif
